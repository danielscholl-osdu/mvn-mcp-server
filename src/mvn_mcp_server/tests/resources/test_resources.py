...
from mvn_mcp_server.resources.triage_reports import (
    TriageReportResource, TriageReport, TriageMetadata, 
    VulnerabilityFinding, DependencyUpdate
)
from mvn_mcp_server.resources.update_plans import (
    UpdatePlanResource, UpdatePlan, PlanMetadata, 
    UpdateTask, PlanPhase, TaskStatus, UpdatePriority
)
...
class TestTriageReportResource:
    ...
    async def test_save_and_retrieve_report(self, triage_resource, sample_report_data):
        ...
        # Save report
        saved_report = await triage_resource.save_report(service_name, sample_report_data)
    ...
    @pytest.mark.asyncio
    async def test_report_metadata_generation(self, triage_resource, sample_report_data):
        ...
        with patch('mvn_mcp_server.resources.triage_reports.datetime') as mock_datetime:
            mock_datetime.now.return_value.isoformat.return_value = "2024-01-24T10:00:00"
    ...
    async def test_get_report_summary(self, triage_resource, sample_report_data):
        ...
        assert len(summary["high_priority_updates"]) == 1  # MAJOR update with age > 12 months
...
class TestUpdatePlanResource:
    ...
    async def test_save_and_retrieve_plan(self, plan_resource, sample_plan_data):
        ...
        # Save plan
        saved_plan = await plan_resource.save_plan(service_name, sample_plan_data, triage_report_id)
    ...
    async def test_plan_metadata_generation(self, plan_resource, sample_plan_data):
        ...
        with patch('mvn_mcp_server.resources.update_plans.datetime') as mock_datetime:
            mock_datetime.now.return_value.isoformat.return_value = "2024-01-24T11:00:00"
    ...
    async def test_task_status_update(self, plan_resource, sample_plan_data):
        ...
        # Update task status
        success = await plan_resource.update_task_status(service_name, "task-1", TaskStatus.COMPLETED)
